...
class _QuizWidgetState extends State<QuizWidget> {
  int _currentQuestionIndex =Random().nextInt(20), total=0,correct=0,current_question=0;
  String msg="";
...
Future<bool> _checkquestion(int number) async {
  ...
  void _nextQuestion() async{
    ...
    if (await _checkquestion( _currentQuestionIndex)){
    setState(() {
      _currentQuestionIndex = (_currentQuestionIndex +1+Random().nextInt(5)) % widget.questions.length;
    ...
    print("Skipped question id: ${_currentQuestionIndex}");
    _nextQuestion();
  }
  }
                              ...
                              child: ListTile(              
                                ...
                                title: Container(
                                  decoration: BoxDecoration(color:_showAnswer && _selectedAnswerIndex == index + 1
                                            ...
                                            && _showAnswer ? const Color.fromARGB(255, 61, 176, 64).withOpacity(0.3) : Color.fromARGB(255, 203, 202, 202)),
...
//            style: ButtonStyle(backgroundColor:MaterialStatePropertyAll(Colors.blueGrey)),
      ...
      List.generate(
  answered.length > 14 ? 14 : answered.length,
  (index) => Icon(
    answered[(answered.length - 14 + index)<0?index:answered.length - 14 + index] ? Icons.check : Icons.clear,
    color: answered[(answered.length - 14 + index)<0?index:answered.length - 14 + index] ? Colors.green : Colors.red,
  ),
),
